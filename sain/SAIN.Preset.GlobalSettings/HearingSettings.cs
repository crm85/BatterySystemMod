...
public class HearingSettings : SAINSettingsBase<HearingSettings>, ISAINSettings
{
	...
	[Description("The Maximum Range that a bot can hear footsteps, sprinting, and jumping, turning, gear sounds, and any movement related sounds, in meters. This is a theoretical max range, actual range heavily changes depending on conditions.")]
	[Category("Hearing Distance")]
	...
	[Description("Higher = Less Randomization and more accuracy. The distance to the sound's position, in meters, is divided by the number here. Example: A unsuppressed gunshot is 150 meters away. And the dispersion value for unsuppressed gunfire is 20. So We divide 150 by 20 to result in 7.5, so the randomized position that a bot thinks a gunshot came from is a position within 7.5 meters from the actual source of the gunshot. Note: this randomized position must be somewhere that is walkable so that they can potentially be able the investigate it. It is also not randomized in height to avoid bots having difficulty navigating to where they think a sound came from, so imagine a flat plane around you that extends 7.5 meters away that includes all walkable space, if you shoot - that bot 150 away will estimate you are somewhere within that 7.5 meter radius, on the same height level that you are on. If there is no walkable space around you, it will find the closest walkable place.")]
	[Category("Position Randomization")]
	[Advanced]
	[MinMax(1f, 100f, 1000f)]
	[DefaultDictionary("HEAR_DISPERSION_VALUES_Defaults")]
	...
	[Description("When a bot has a bullet fly by them, the dispersion on the source of the gunshot will be X times more random, so a value of 2 will mean that it will multiply the randomized position to be 2x as far as originally calculated.")]
	[Category("Position Randomization")]
	[Advanced]
	[MinMax(1f, 10f, 100f)]
	...
	[Description("If a bot is looking at the source of a sound, they will be more accurate in their position prediction, up to the Minimum value here. If it is directly behind them, randomization will be multiplied by the Maximumm value here. It is a linear scale between these, so a sound directly to their right or left, will have the difference between the Maximumm and Minimum. Setting both the Min and the Max to 1.0 will disable this system.")]
	[Category("Position Randomization")]
	[Advanced]
	...
	[Advanced]
	[MinMax(0f, 100f, 100f)]
	public float HEAR_CHANCE_MIN_DIST_HEADPHONES = 1f;
	...
	[Advanced]
	[MinMax(0f, 100f, 1f)]
	public float HEAR_CHANCE_MIDRANGE_MINCHANCE_HEADPHONES = 3f;
	...
	[Advanced]
	[MinMax(0f, 100f, 1f)]
	public float HEAR_CHANCE_LONGRANGE_MINCHANCE_HEADPHONES = 1f;
	...
	[Advanced]
	[MinMax(0f, 100f, 1f)]
	public float HEAR_CHANCE_NOTMOVING_MINCHANCE = 2f;

	[Name("Standing Still Min Chance - Headphones")]
	[Description("If a bot is standing still, add X percent to a bot minimum hear chance for sounds that aren't gunshots.")]
	[Category("Hearing Chance")]
	[MinMax(0f, 100f, 1f)]
	public float HEAR_CHANCE_NOTMOVING_MINCHANCE_HEADPHONES = 4f;
	...
	[Advanced]
	[MinMax(0f, 100f, 1f)]
	public float HEAR_CHANCE_HEADPHONES_OTHERSOUNDS = 3f;
	...
	[Advanced]
	[MinMax(0f, 100f, 1f)]
	public float HEAR_CHANCE_CURRENTENEMY_MINCHANCE = 2f;
	...
	[Advanced]
	[MinMax(0f, 100f, 1f)]
	public float HEAR_CHANCE_CURRENTENEMY_MINCHANCE_HEADPHONES = 3f;
	...
	[Advanced]
	public float MaxSoundRange_FallLanding = 70f;
	...
	public HearingDelaySettings BaseHearingDelaySettings = new HearingDelaySettings(0.5f, 0.1f, 0.25f, 0.66f);
	...
	[Category("Hearing Delay / Reaction Delay")]
	[Advanced]
	public Dictionary<SAINSoundType, HearingDelaySettings> HEARINGDELAY_SETTINGS = new Dictionary<SAINSoundType, HearingDelaySettings>
	{
		{
			SAINSoundType.Shot,
			new HearingDelaySettings(0.25f, 0.1f, 0.2f, 0.6f)
		},
		{
			SAINSoundType.SuppressedShot,
			new HearingDelaySettings(0.3f, 0.1f, 0.25f, 0.65f)
		},
		{
	...
	[MinMax(30f, 400f, 10f)]
	[Advanced]
	[DefaultDictionary("HearingDistancesDefaults")]
	public Dictionary<ECaliber, float> HearingDistances = new Dictionary<ECaliber, float>
	{
		{
			ECaliber.Caliber9x18PM,
			110f
		},
		{
			ECaliber.Caliber9x19PARA,
			110f
		},
		{
			ECaliber.Caliber46x30,
			120f
		},
		{
			ECaliber.Caliber9x21,
			120f
		},
		{
			ECaliber.Caliber57x28,
			120f
		},
		{
			ECaliber.Caliber762x25TT,
			120f
		},
		{
			ECaliber.Caliber1143x23ACP,
			115f
		},
		{
			ECaliber.Caliber9x33R,
			125f
		},
		{
			ECaliber.Caliber545x39,
			160f
		},
		{
			ECaliber.Caliber556x45NATO,
			160f
		},
		{
			ECaliber.Caliber9x39,
			160f
		},
		{
			ECaliber.Caliber762x35,
			175f
		},
		{
			ECaliber.Caliber762x39,
			175f
		},
		{
			ECaliber.Caliber366TKM,
			175f
		},
		{
			ECaliber.Caliber762x51,
			200f
		},
		{
			ECaliber.Caliber127x55,
			200f
		},
		{
			ECaliber.Caliber762x54R,
			225f
		},
		{
			ECaliber.Caliber86x70,
			250f
		},
		{
			ECaliber.Caliber20g,
		...
		{
			ECaliber.Caliber23x75,
			210f
		},
		{
			ECaliber.Caliber26x75,
			50f
		},
		{
			ECaliber.Caliber30x29,
			50f
		},
		{
			ECaliber.Caliber40x46,
			50f
		},
		{
			ECaliber.Caliber40mmRU,
			50f
		},
		{
			ECaliber.Caliber127x108,
			300f
		},
		{
			ECaliber.Caliber68x51,
			200f
		},
		{
			ECaliber.Default,
	...
	[JsonIgnore]
	[Hidden]
	public static readonly Dictionary<ECaliber, float> HearingDistancesDefaults;

	[JsonIgnore]
	[Hidden]
	private const string _dispersion_descr = "Higher = Less Randomization and more accuracy. The distance to the sound's position, in meters, is divided by the number here. Example: A unsuppressed gunshot is 150 meters away. And the dispersion value for unsuppressed gunfire is 20. So We divide 150 by 20 to result in 7.5, so the randomized position that a bot thinks a gunshot came from is a position within 7.5 meters from the actual source of the gunshot. Note: this randomized position must be somewhere that is walkable so that they can potentially be able the investigate it. It is also not randomized in height to avoid bots having difficulty navigating to where they think a sound came from, so imagine a flat plane around you that extends 7.5 meters away that includes all walkable space, if you shoot - that bot 150 away will estimate you are somewhere within that 7.5 meter radius, on the same height level that you are on. If there is no walkable space around you, it will find the closest walkable place.";

	[JsonIgnore]
	[Hidden]
	private const string _hear_angle_descr = "If a bot is looking at the source of a sound, they will be more accurate in their position prediction, up to the Minimum value here. If it is directly behind them, randomization will be multiplied by the Maximumm value here. It is a linear scale between these, so a sound directly to their right or left, will have the difference between the Maximumm and Minimum. Setting both the Min and the Max to 1.0 will disable this system.";
	...
	static HearingSettings()
	{
		...
		ListHelpers.PopulateKeys(HEAR_DISPERSION_VALUES_Defaults, 12.5f);
		HearingDistancesDefaults = new Dictionary<ECaliber, float>
		{
			{
				ECaliber.Caliber9x18PM,
		...
		ListHelpers.PopulateKeys(HearingDistancesDefaults, 125f);
	}
	...
	{
		ListHelpers.CloneEntries(HEAR_DISPERSION_VALUES_Defaults, HEAR_DISPERSION_VALUES);
		ListHelpers.CloneEntries(HearingDistancesDefaults, HearingDistances);
		list.Add(this);
	}
}
