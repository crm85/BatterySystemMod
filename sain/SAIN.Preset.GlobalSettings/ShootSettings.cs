...
public class ShootSettings : SAINSettingsBase<ShootSettings>, ISAINSettings
{
	...
	[MinMax(25f, 300f, 1f)]
	[Advanced]
	public float RECOIL_BASELINE = 100f;
	...
	[Description("Lower is BETTER. How Shootable this ammo type is, affects semi auto firerate and full auto burst length.Value is scaled but roughly gives a plus or minus 20% to firerate depending on the value set here.For Example. 9x19 will shoot about 20% faster fire-rate on semi-auto at 50 meters, and fire 20% longer bursts when on full auto")]
	[Category("Bot Weapon Control")]
	[Percentage0to1(0.01f, 1f, 100f)]
	[Advanced]
	[DefaultDictionary("AmmoCaliberShootabilityDefaults")]
	...
	[Advanced]
	[DefaultDictionary("AmmoCaliberFullAutoMaxDistancesDefaults")]
	...
	[Description("Lower is BETTER. How Shootable this weapon type is, affects semi auto firerate and full auto burst length.Value is scaled but roughly gives a plus or minus 20% to firerate depending on the value set here.For Example. SMGs will shoot about 20% faster fire-rate on semi-auto at 50 meters, and fire 20% longer bursts when on full auto")]
	[Category("Bot Weapon Control")]
	[Percentage0to1(0.01f, 1f, 100f)]
	[Advanced]
	[DefaultDictionary("WeaponClassShootabilityDefaults")]
	public Dictionary<EWeaponClass, float> WeaponClassShootability = new Dictionary<EWeaponClass, float>
	{
		{
			EWeaponClass.Default,
			0.425f
		},
		{
			EWeaponClass.assaultCarbine,
			0.5f
		},
		{
			EWeaponClass.assaultRifle,
			0.5f
		},
		{
			EWeaponClass.machinegun,
			0.15f
		},
		{
			EWeaponClass.smg,
		...
		{
			EWeaponClass.marksmanRifle,
			0.75f
		},
		{
			EWeaponClass.sniperRifle,
			1f
		},
		{
			EWeaponClass.shotgun,
			0.75f
		},
		{
			EWeaponClass.grenadeLauncher,
			1f
		},
		{
			EWeaponClass.specialWeapon,
			1f
		}
	};
	...
	[Description("HIGHER is BETTER. This is the time to wait inbetween shots for every meter.the number is divided by the distance to their target, to get a wait period between shots.For Example. With a setting of 100: if a target is 50m away, they will wait 0.5 sec between shots because 50 / 100 is 0.5.This number is later modified by the Shootability multiplier, to get a final fire-rate that gets sent to a bot.")]
	[MinMax(30f, 250f, 1f)]
	[Category("Bot Weapon Control")]
	[Advanced]
	[DefaultDictionary("WeaponPerMeterDefaults")]
	...
	[Description("The distances that a bot prefers to shoot a particular weapon class. Bots will try to close the distance if further than this.")]
	[MinMax(10f, 250f, 1f)]
	[Category("Bot Weapon Control")]
	[Advanced]
	[DefaultDictionary("EngagementDistanceDefaults")]
	...
	[Description("Affects Weapon Shootability Calculations, which tells a bot how fast they should shoot, how long they should hold down full auto fire, and how far they will decide to fire full auto or swap to only semi-auto.. Affects how much this type of effect has on how Shootable a bot's weapon is considered. Higher = More effect. So if you set RecoilScaling to 0, recoil will play no part in how fast or slow a bot shoots.")]
	[Category("Bot Weapon Control")]
	[Advanced]
	[Percentage01to99]
