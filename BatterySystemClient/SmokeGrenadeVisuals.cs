...
namespace BatterySystem
{
	public class SmokeGrenadeExplosionPatch : ModulePatch
	{
		...
		{
			if (__instance == null || !BatterySystemPlugin.InGame()) return;

			if (BatterySystemConfig.EnableSmokeOccluders.Value
	...
	public class M18SmokeEffectSuppressionPatch : ModulePatch
	{
		...
		{
			if (!BatterySystemConfig.ReplaceM18SmokeVisual.Value) return true;
			...
			{
				Debug.Log("[BatterySystem] Suppressing vanilla M18 smoke effect; using airdrop plume replacement.");
	...
	internal static class SmokeVisualReplacement
	{
		...
		{
			return BatterySystemConfig.ReplaceM18SmokeVisual.Value
				&& BatterySystemPlugin.InGame()
	...
	internal sealed class SmokeOcclusionController : MonoBehaviour
	{
		...
		public void Init(SmokeGrenade grenade)
		{
			...
			if (foliageLayer < 0)
			{
				...
				{
					Debug.LogWarning("[BatterySystem] Smoke occluders disabled: Foliage layer was not found.");
			...
			_radiusMultiplier = BatterySystemConfig.SmokeOccluderRadiusMultiplier.Value
				* (isM18 ? BatterySystemConfig.M18SmokeOccluderRadiusMultiplier.Value : 1f);
			_heightMultiplier = isM18 ? 1.35f : 1f;

			_root = new GameObject("BatterySystem_SmokeOccluders");
			_root.layer = foliageLayer;
			_root.transform.SetPositionAndRotation(grenade.transform.position, Quaternion.identity);

			int baseCount = Mathf.Clamp(BatterySystemConfig.SmokeOccluderDensity.Value, 4, 48);
		...
		private void Update()
		{
			...
			bool showDebugVisuals = BatterySystemConfig.SmokeOccluderDebugVisuals.Value;
	...
	internal sealed class M18AirdropSmokeController : MonoBehaviour
	{
		...
		public void Init(SmokeGrenade grenade)
		{
			...
			_root = new GameObject("BatterySystem_M18AirdropSmoke");
			_root.transform.SetPositionAndRotation(grenade.transform.position, Quaternion.identity);
		...
		{
			int plumeCount = Mathf.Clamp(BatterySystemConfig.M18AirdropPlumeCount.Value, 1, 8);
			float plumeScale = Mathf.Clamp(BatterySystemConfig.M18AirdropPlumeScale.Value, 0.5f, 4f);
			...
			Debug.Log("[BatterySystem] Spawned " + spawned + " airdrop smoke plume(s) for M18 smoke.");
		}
	...
	internal static class AirdropSmokeVisualCache
	{
		...
		internal static bool TryEnsurePrefab()
		{
			...
			try
			{
				...
				_plumePrefab.name = "BatterySystem_AirdropSmokePlumePrefab";
				SanitizeNonVisualComponents(_plumePrefab);
				StripIgnitionVisuals(_plumePrefab);
				_plumePrefab.SetActive(false);
				Debug.Log("[BatterySystem] Loaded airdrop flare visual for M18 smoke replacement.");
				return true;
			}
			catch (Exception ex)
			{
			...
			finally
			{
				if (syncObject != null)
				{
					...
					{
						Debug.LogWarning("[BatterySystem] Failed to return temporary airdrop object to pool: " + ex.Message);
					}
				}
			}
		}
		...
		internal static GameObject InstantiatePlume(Transform parent)
		{
			...
			plume.name = "BatterySystem_M18_AirdropSmokePlume";
		...
		internal static void ConfigureRuntimePlume(GameObject plume, float scale)
		{
			...
			float smokeAmount = Mathf.Clamp(BatterySystemConfig.M18AirdropSmokeAmount.Value, 0.5f, 6f);
		...
		private static bool FailLoad(string reason)
		{
			...
			{
				Debug.LogWarning("[BatterySystem] M18 airdrop smoke replacement unavailable; vanilla M18 smoke will be used. " + reason);
		...
		private static void StripIgnitionVisuals(GameObject obj)
		{
			...
			{
				Debug.Log("[BatterySystem] Disabled " + stripped + " non-smoke flare/fire visual component(s) from M18 airdrop smoke.");
