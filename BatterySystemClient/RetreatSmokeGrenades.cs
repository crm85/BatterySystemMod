...
namespace BatterySystem
{
	public class SainRetreatDecisionSmokePatch : ModulePatch
	{
		...
		public static void TryEnable()
		{
			...
			Debug.Log("[BatterySystem] Enabled SAIN retreat smoke grenade patch.");
		}
		...
		private static void Postfix(object __instance, string __state)
		{
			...
			catch (Exception ex)
			{
				...
				Debug.LogWarning("[BatterySystem] SAIN retreat smoke patch failed at runtime; disabling logs for further failures. " + ex);
	...
	internal static class RetreatSmokeGrenadeThrower
	{
		...
		{
			if (!BatterySystemConfig.EnableRetreatSmokeGrenades.Value) return;
		...
		private static bool TryThrowRetreatSmoke(BotOwner botOwner, out string reason)
		{
			...
			float chance = Mathf.Clamp01(BatterySystemConfig.RetreatSmokeChance.Value);
			...
			out AIGreanageThrowData throwData)
		{
			...
			if (BatterySystemConfig.RetreatSmokeAllowEmergencyToss.Value)
			{
		...
		private static Vector3[] BuildTargetCandidates(BotOwner botOwner, Vector3 target)
		{
			...
			Vector3 far = botPosition + direction * Mathf.Min(distance + 2f, BatterySystemConfig.RetreatSmokeThrowDistance.Value) + Vector3.up * 0.35f;
		...
		private static Vector3 GetSmokeTarget(BotOwner botOwner, Vector3 from)
		{
			...
			if (TryGetEnemyPosition(botOwner, out Vector3 enemyPosition))
			{
				...
				if (toEnemy.sqrMagnitude > 0.01f)
				{
					...
					float maxThrowDistance = BatterySystemConfig.RetreatSmokeThrowDistance.Value;
			...
			return from + lookDirection.normalized * Mathf.Min(4f, BatterySystemConfig.RetreatSmokeThrowDistance.Value);
		}
		...
		{
			NextAttemptTimes[botOwner] = Time.time + Mathf.Max(1f, BatterySystemConfig.RetreatSmokeCooldown.Value);
		}
		...
		{
			if (!BatterySystemConfig.RetreatSmokeDebugLogging.Value) return;

			string botName = botOwner != null
				? botOwner.name
				: "unknown bot";
			Debug.Log("[BatterySystem] " + botName + " " + message);
		}
	}
}
